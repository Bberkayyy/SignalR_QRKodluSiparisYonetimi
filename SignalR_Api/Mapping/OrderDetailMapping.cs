...
public class OrderDetailMapping : Profile
{
    public OrderDetailMapping()
    {
        ...
        CreateMap<OrderDetail, GetAllOrderDetailWithRelationshipsResponseDto>().ForMember(x => x.ProductName, opt => opt.MapFrom(x => x.Product.Name)).ForMember(x => x.OrderTableName, opt => opt.MapFrom(x => x.Order.TableName)).ReverseMap();
        CreateMap<OrderDetail, GetOrderDetailWithRelationshipsResponseDto>().ForMember(x => x.ProductName, opt => opt.MapFrom(x => x.Product.Name)).ForMember(x => x.OrderTableName, opt => opt.MapFrom(x => x.Order.TableName)).ReverseMap();
    }
}
